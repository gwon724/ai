  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", backgroundColor: "#0F172A", fontFamily: font }}>
        ...
        <div style={{ maxWidth: "1300px", margin: "0 auto", padding: "16px 12px" }}>
          {/* Stats */}
          ...
          {/* Grade summary */}
          <div style={{ backgroundColor: "#1E293B", borderRadius: "12px", border: "1px solid #334155", padding: "14px 18px", marginBottom: "14px" }}>
            <p style={{ fontSize: "13px", fontWeight: "700", color: "#CBD5E1", marginBottom: "10px" }}>📊 SOHO 등급별 자금 현황</p>
            <div className="fp-grade-row">
              {GRADES.map(g => {
                const gFunds = funds.filter(f => f.active && f.eligibleGrades.includes(g));
                return (
                  <div key={g} className="fp-grade-col" style={{ backgroundColor: "#0F172A", borderRadius: "8px", padding: "10px 12px", border: `1px solid ${GRADE_COLORS[g]}40` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                      <span style={{ fontSize: "18px", fontWeight: "800", color: GRADE_COLORS[g] }}>{g}등급</span>
                      <span style={{ fontSize: "16px", fontWeight: "700", color: "#F1F5F9" }}>{gFunds.length}개</span>
                    </div>
                    {gFunds.slice(0, 2).map(f => (
                      <p key={f.id} style={{ fontSize: "11px", color: "#64748B", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>• {f.name}</p>
                    ))}
                    {gFunds.length > 2 && <p style={{ fontSize: "11px", color: "#475569" }}>+{gFunds.length - 2}개 더</p>}
                    {gFunds.length === 0 && <p style={{ fontSize: "11px", color: "#334155" }}>없음</p>}
                  </div>
                );
              })}
            </div>
          ...
          <div className="fp-filter-row">
            <input placeholder="🔍 자금명 또는 기관명" value={search} onChange={e => setSearch(e.target.value)}
              style={{ flex: "1", minWidth: "150px", padding: "9px 12px", fontSize: "13px", border: "1px solid #334155", borderRadius: "8px", backgroundColor: "#0F172A", color: "#F1F5F9", outline: "none", fontFamily: font }} />
            <select value={filterGrade} onChange={e => setFilterGrade(e.target.value)}
              style={{ padding: "9px 10px", fontSize: "13px", border: "1px solid #334155", borderRadius: "8px", backgroundColor: "#0F172A", color: "#F1F5F9", outline: "none", cursor: "pointer", fontFamily: font }}>
            ...
            <p style={{ fontSize: "12px", color: "#64748B" }}>{filtered.length}개</p>
            <button onClick={openAdd} style={{ marginLeft: "auto", padding: "9px 16px", backgroundColor: "#2563EB", color: "#FFFFFF", fontSize: "13px", fontWeight: "700", border: "none", borderRadius: "8px", cursor: "pointer", fontFamily: font, whiteSpace: "nowrap" }}>
              + 자금 추가
            </button>
          </div>
            ...
            {filtered.map(f => {
              ...
              return (
                <div key={f.id} style={{ backgroundColor: "#1E293B", borderRadius: "12px", border: `1px solid ${f.active ? "#334155" : "#1E293B"}`, overflow: "hidden", opacity: f.active ? 1 : 0.55 }}>
                  <div style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: "10px", cursor: "pointer", flexWrap: "wrap" }}
                    onClick={() => setExpandedId(isExpanded ? null : f.id)}>
                    <div style={{ display: "flex", gap: "3px", flexShrink: 0 }}>
                      {GRADES.map(g => (
                        <span key={g} style={{ fontSize: "11px", fontWeight: "700", padding: "2px 6px", borderRadius: "5px", backgroundColor: f.eligibleGrades.includes(g) ? `${GRADE_COLORS[g]}22` : "#0F172A", color: f.eligibleGrades.includes(g) ? GRADE_COLORS[g] : "#334155", border: `1px solid ${f.eligibleGrades.includes(g) ? GRADE_COLORS[g] + "60" : "#1E293B"}` }}>{g}</span>
                      ))}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                        <p style={{ fontSize: "14px", fontWeight: "700", color: "#F1F5F9" }}>{f.name}</p>
                        {!f.active && <span style={{ fontSize: "10px", color: "#64748B", backgroundColor: "#334155", padding: "2px 6px", borderRadius: "4px" }}>비활성</span>}
                      </div>
                      <p style={{ fontSize: "12px", color: "#64748B", marginTop: "2px" }}>{f.institution} · {f.category} · 최대 {Number(f.maxAmount).toLocaleString()}원 · {f.interestRate}</p>
                    </div>
                    <div style={{ display: "flex", gap: "5px", flexShrink: 0, flexWrap: "wrap" }}>
                      <button onClick={e => { e.stopPropagation(); updateFund(f.id, { active: !f.active }); refresh(); }}
                        style={{ fontSize: "11px", fontWeight: "600", color: f.active ? "#FBBF24" : "#22C55E", border: `1px solid ${f.active ? "#78350F" : "#14532D"}`, backgroundColor: f.active ? "#1A0F00" : "#052E1C", padding: "4px 8px", borderRadius: "6px", cursor: "pointer" }}>
                        {f.active ? "⏸" : "▶"}
                      </button>
                      <button onClick={e => { e.stopPropagation(); openEdit(f); }}
                        style={{ fontSize: "11px", fontWeight: "600", color: "#60A5FA", border: "1px solid #1E3A5F", backgroundColor: "#0F172A", padding: "4px 8px", borderRadius: "6px", cursor: "pointer" }}>
                        ✏️
                      </button>
                      <button onClick={e => { e.stopPropagation(); setDeleteConfirm(f.id); }}
                        style={{ fontSize: "11px", fontWeight: "600", color: "#F87171", border: "1px solid #450A0A", backgroundColor: "#1A0505", padding: "4px 7px", borderRadius: "6px", cursor: "pointer" }}>
                        🗑️
                      </button>
                      <span style={{ fontSize: "13px", color: "#475569", padding: "4px 4px" }}>{isExpanded ? "▲" : "▼"}</span>
                    </div>
                  </div>

                  {isExpanded && (
                    <div style={{ borderTop: "1px solid #0F172A", padding: "14px 16px", backgroundColor: "#0F172A" }}>
                      <div className="fund-detail-grid">
                        {[
                          ...
                          <div key={item.label}>
                            <p style={{ fontSize: "10px", color: "#64748B", marginBottom: "3px" }}>{item.label}</p>
                            <p style={{ fontSize: "13px", fontWeight: "600", color: "#E2E8F0" }}>{item.value}</p>
                          </div>
                        ))}
                      </div>
                      {f.description && (
                        <div style={{ backgroundColor: "#1E293B", borderRadius: "8px", padding: "10px 12px" }}>
                          <p style={{ fontSize: "11px", color: "#64748B", marginBottom: "4px" }}>자금 설명</p>
                          <p style={{ fontSize: "13px", color: "#CBD5E1", lineHeight: "1.7" }}>{f.description}</p>
                        </div>
                      )}
                    </div>
                  )}
        ...
        {/* Add/Edit Modal */}
        {showForm && (
          <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.65)", display: "flex", alignItems: "flex-start", justifyContent: "center", zIndex: 50, padding: "16px", overflowY: "auto" }}>
            <div style={{ backgroundColor: "#1E293B", borderRadius: "16px", padding: "24px", width: "100%", maxWidth: "580px", border: "1px solid #334155", boxShadow: "0 8px 40px rgba(0,0,0,0.5)", marginBottom: "20px" }}>
              <h3 style={{ fontSize: "16px", fontWeight: "700", color: "#F1F5F9", marginBottom: "18px", fontFamily: font }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  ...
                  <button type="submit"
                    style={{ flex: 2, padding: "11px", backgroundColor: "#2563EB", color: "#FFFFFF", fontSize: "13px", fontWeight: "700", border: "none", borderRadius: "8px", cursor: "pointer" }}>
        ...
        {deleteConfirm && (
          <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: "16px" }}>
            <div style={{ backgroundColor: "#1E293B", borderRadius: "14px", padding: "28px", maxWidth: "340px", width: "100%", border: "1px solid #334155" }}>
              ...
              <p style={{ fontSize: "15px", fontWeight: "700", color: "#F1F5F9", textAlign: "center", marginBottom: "8px" }}>자금을 삭제하시겠습니까?</p>
              <p style={{ fontSize: "13px", color: "#94A3B8", textAlign: "center", marginBottom: "24px" }}>{funds.find(f => f.id === deleteConfirm)?.name}</p>
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                <button onClick={() => { deleteFund(deleteConfirm); refresh(); setDeleteConfirm(null); flash(); }}
                  style={{ flex: 1, padding: "11px", backgroundColor: "#DC2626", color: "#FFFFFF", fontSize: "13px", fontWeight: "700", border: "none", borderRadius: "8px", cursor: "pointer" }}>삭제</button>
              </div>
            </div>
          </div>
        )}
