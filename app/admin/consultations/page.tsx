    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        .cp-header { background-color:#1E293B; border-bottom:1px solid #334155; padding:0 12px; position:sticky; top:0; z-index:50; }
        .cp-header-inner { max-width:1400px; margin:0 auto; display:flex; justify-content:space-between; align-items:center; height:52px; gap:8px; }
        .cp-brand { text-decoration:none; display:flex; align-items:center; gap:8px; flex-shrink:0; }
        .cp-brand span { font-size:14px; font-weight:800; color:#F1F5F9; white-space:nowrap; }
        .cp-nav-links { display:flex; gap:12px; align-items:center; }
        .cp-nav-links a { font-size:12px; color:#94A3B8; text-decoration:none; font-family:${font}; white-space:nowrap; }
        ...
        .filter-row { background-color:#1E293B; border-radius:12px; padding:10px 12px; border:1px solid #334155; display:flex; gap:8px; flex-wrap:wrap; align-items:center; }
        /* Detail panel — fullscreen on mobile */
        .detail-panel-overlay { display:none; }
        @media (max-width:1023px) {
          .detail-panel-overlay.open { display:flex; position:fixed; inset:0; background:rgba(15,23,42,0.98); z-index:80; overflow-y:auto; padding:12px; flex-direction:column; }
          .detail-panel-inline { display:none !important; }
        }
        @media (min-width:1024px) {
          .detail-panel-inline { display:block; }
          .detail-panel-overlay { display:none !important; }
        }
        ...
        .detail-tab-btn { padding:8px 10px; border:none; cursor:pointer; font-family:${font}; font-size:12px; font-weight:700; border-radius:8px 8px 0 0; border-bottom:2px solid transparent; background:transparent; color:#64748B; }
        .detail-tab-btn.active { background:#0F172A; color:#60A5FA; border-bottom:2px solid #2563EB; }
        .cp-main { max-width:1400px; width:100%; margin:0 auto; padding:14px 10px; }
        .cp-layout { display:flex; gap:14px; }
        .cp-list-col { flex:1; display:flex; flex-direction:column; gap:10px; min-width:0; }
        .cp-detail-col { flex:0 0 440px; display:flex; flex-direction:column; gap:12px; }
        @media (max-width:1023px) { .cp-layout { flex-direction:column; } .cp-detail-col { flex:none; } }
      `}</style>
      ...
      <div style={{ minHeight: "100vh", backgroundColor: "#0F172A", fontFamily: font, display: "flex", flexDirection: "column" }}>
        ...
        <div className={`mobile-overlay ${mobileNav ? "open" : ""}`}>
          ...
          <Link href="/admin/consultations" style={{ background: "#2563EB", color: "#FFF" }} onClick={() => setMobileNav(false)}>💬 상담 관리</Link>
        ...
        <div className="cp-main">
          ...
          <div className="cp-layout">
            ...
            <div className="cp-list-col">
              ...
              <div className="filter-row">
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 이름 / 연락처 / 이메일 / 접수번호"
                  style={{ ...inp, flex: 1, minWidth: "140px", backgroundColor: "#0F172A" }} />
              ...
              {/* List */}
              <div style={{ backgroundColor: "#1E293B", borderRadius: "12px", border: "1px solid #334155", overflow: "hidden", flex: 1 }}>
                {filtered.length === 0 ? (
                      ...
                      <thead>
                        <tr style={{ backgroundColor: "#0F172A", borderBottom: "1px solid #334155" }}>
                          {["접수번호", "신청자", "연락처", "연매출", "상태", "신청일", ""].map(h => (
                            <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: "11px", fontWeight: "600", color: "#64748B", whiteSpace: "nowrap" }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {filtered.map(c => {
                          ...
                          return (
                            <tr key={c.id}
                              ...
                              <td style={{ padding: "10px 12px", fontSize: "12px", color: "#60A5FA", fontWeight: "700", whiteSpace: "nowrap" }}>{c.id}</td>
                              <td style={{ padding: "10px 12px" }}>
                                <p style={{ fontSize: "13px", fontWeight: "700", color: "#F1F5F9" }}>{c.name}</p>
                                <p style={{ fontSize: "11px", color: "#64748B" }}>{c.email}</p>
                              </td>
                              <td style={{ padding: "10px 12px", fontSize: "12px", color: "#CBD5E1", whiteSpace: "nowrap" }}>{c.phone}</td>
                              <td style={{ padding: "10px 12px", fontSize: "12px", color: "#94A3B8", whiteSpace: "nowrap" }}>
                              ...
                              <td style={{ padding: "10px 12px", fontSize: "11px", color: "#64748B", whiteSpace: "nowrap" }}>{c.createdAt.slice(0, 10)}</td>
                              <td style={{ padding: "10px 12px" }}>
            ...
            {selected && showDetail && (
              <div className="cp-detail-col detail-panel-inline">
                <DetailContent
                  ...
                  saved={saved} delConfirm={delConfirm} setDelConfirm={setDelConfirm}
          ...
          {selected && showDetail && (
            <div className={`detail-panel-overlay open`} style={{ paddingTop: "60px" }}>
...
function DetailContent({
  selected, detailTab, setDetailTab, allFunds, newStatus, setNewStatus,
  memo, setMemo, assignedTo, setAssignedTo, consultDate, setConsultDate,
  saved, delConfirm, setDelConfirm, onSave, onDelete, onClose,
  GRADE_COLOR, inp,
}: {
  ...
  setNewStatus: (s: ConsultStatus) => void;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      {/* Header */}
      <div style={{ backgroundColor: "#1E293B", borderRadius: "14px", border: "1px solid #334155", padding: "16px 18px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            <span style={{ padding: "5px 10px", borderRadius: "999px", fontSize: "12px", fontWeight: "700", backgroundColor: CONSULT_STATUS_COLORS[selected.status].darkBg, color: CONSULT_STATUS_COLORS[selected.status].darkText, border: `1px solid ${CONSULT_STATUS_COLORS[selected.status].border}44`, whiteSpace: "nowrap" }}>
              {selected.status}
            </span>
            <button onClick={onClose}
              style={{ width: "28px", height: "28px", backgroundColor: "#334155", border: "none", borderRadius: "50%", color: "#94A3B8", cursor: "pointer", fontSize: "14px", flexShrink: 0 }}>×</button>
          </div>
        </div>

        {/* Tab menu */}
        <div style={{ display: "flex", gap: "4px", borderBottom: "1px solid #334155", overflowX: "auto" }}>
          {([
            { key: "info", label: "📋 기본정보" },
            ...
            <button key={t.key} onClick={() => setDetailTab(t.key)}
              style={{ padding: "8px 10px", border: "none", cursor: "pointer", fontFamily: font, fontSize: "12px", fontWeight: "700", borderRadius: "8px 8px 0 0", borderBottom: `2px solid ${detailTab === t.key ? "#2563EB" : "transparent"}`, background: detailTab === t.key ? "#0F172A" : "transparent", color: detailTab === t.key ? "#60A5FA" : "#64748B", whiteSpace: "nowrap" }}>
              {t.label}
            </button>
          ))}
        </div>
            ...
            <>
              <div style={{ backgroundColor: "#1E293B", borderRadius: "14px", border: "1px solid #334155", padding: "16px 18px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "10px" }}>
                  <div style={{ width: "52px", height: "52px", borderRadius: "50%", backgroundColor: GRADE_COLOR[ai.sohoGrade]?.bg || "#0F172A", border: `3px solid ${GRADE_COLOR[ai.sohoGrade]?.text || "#475569"}66`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      ...
      <div style={{ backgroundColor: "#1E293B", borderRadius: "14px", border: "1px solid #334155", padding: "16px 18px" }}>
          ...
          {CONSULT_STATUS_LIST.map(s => {
            ...
            return (
              <button key={s} onClick={() => setNewStatus(s)}
                style={{ padding: "5px 10px", borderRadius: "8px", fontSize: "11px", fontWeight: "700", cursor: "pointer", border: `2px solid ${isActive ? sc.border : "#334155"}`, backgroundColor: isActive ? sc.darkBg : "#0F172A", color: isActive ? sc.darkText : "#64748B" }}>
                {s}
              </button>
            );
          })}
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          <button onClick={onSave}
            style={{ flex: 1, padding: "11px", border: "none", borderRadius: "10px", fontSize: "13px", fontWeight: "700", cursor: "pointer", backgroundColor: saved ? "#16A34A" : "#2563EB", color: "#FFF" }}>
            {saved ? "✓ 저장됨" : "💾 저장"}
          </button>
          <button onClick={() => setDelConfirm(true)}
            style={{ padding: "11px 14px", border: "1px solid #450A0A", borderRadius: "10px", fontSize: "13px", fontWeight: "700", cursor: "pointer", backgroundColor: "#1A0505", color: "#F87171" }}>
            🗑️
          </button>
        </div>

        {delConfirm && (
          <div style={{ marginTop: "12px", backgroundColor: "#1A0505", borderRadius: "10px", border: "1px solid #7F1D1D", padding: "14px" }}>
            <p style={{ fontSize: "13px", color: "#FCA5A5", fontWeight: "700", marginBottom: "10px" }}>정말 삭제하시겠습니까?</p>
            <div style={{ display: "flex", gap: "8px" }}>
              <button onClick={() => setDelConfirm(false)} style={{ flex: 1, padding: "8px", backgroundColor: "#334155", color: "#CBD5E1", border: "none", borderRadius: "7px", cursor: "pointer", fontSize: "12px", fontWeight: "600" }}>취소</button>
              <button onClick={onDelete} style={{ flex: 1, padding: "8px", backgroundColor: "#DC2626", color: "#FFF", border: "none", borderRadius: "7px", cursor: "pointer", fontSize: "12px", fontWeight: "700" }}>삭제</button>
            </div>
          </div>
        )}
      </div>
